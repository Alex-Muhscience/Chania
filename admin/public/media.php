...
if (!empty($search)) {
    $mediaItems = $media->search($search, $filter, $sort, $limit, $offset);
    $totalItems = $media->getSearchCount($search, $filter);
} else {
    $mediaItems = $media->getFiltered($filter, $sort, $limit, $offset);
    $totalItems = $media->getFilteredCount($filter);
}
...
if ($_POST['action'] ?? '' === 'bulk_delete' && !empty($_POST['selected_items'])) {
    try {
        ...
        $_SESSION['success_message'] = "Successfully deleted {$deletedCount} media file(s).";
    } catch (Exception $e) {
        $_SESSION['error_message'] = 'Error deleting media files: ' . $e->getMessage();
    }
                ...
                <form method="GET" class="row align-items-end">
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control" id="search" name="search" 
                               placeholder="Search by filename..." value="<?= htmlspecialchars($search) ?>">
                    </div>
                    <div class="col-md-3">
                                            ...
                                            <?php foreach ($mediaItems as $item) : ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <a href="<?= BASE_URL ?>/admin/actions/delete_media.php?id=<?= $item['id'] ?>" 
                                                           class="btn btn-danger btn-sm" 
                                                           onclick="return confirm('Are you sure you want to delete this media file?')" title="Delete">
                                ...
                                <?php if ($totalPages > 1): ?>
                                    <nav aria-label="Media pagination" class="mt-4">
                                        ...
                                        <div class="text-center text-muted">
                                            Showing <?= min(($page - 1) * $limit + 1, $totalItems) ?> to <?= min($page * $limit, $totalItems) ?> of <?= number_format($totalItems) ?> files
                                        </div>
                                    </nav>
                                <?php endif; ?>
