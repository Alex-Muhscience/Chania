...
if (!$template) {
    $_SESSION['error'] = "SMS template not found.";
    Utilities::redirect('/admin/public/sms_templates.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $content) {
        $stmt = $db->prepare("UPDATE sms_templates SET name = ?, content = ?, template_type = ? WHERE id = ?");
        if ($stmt->execute([$name, $content, $template_type, $template_id])) {
            $_SESSION['message'] = "SMS template updated successfully.";
            Utilities::redirect('/admin/public/sms_templates.php');
            exit();
        } else {
...
<div class="container-fluid px-4">
    <h1 class="mt-4">Edit SMS Template</h1>
    ...
    <form action="" method="post">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="content" name="content" rows="4" required><?= htmlspecialchars($template['content']); ?></textarea>
        </div>
        <div class="mb-3">
            ...
            <select class="form-select" id="template_type" name="template_type">
                ...
                <option value="announcement" <?= $template['template_type'] === 'announcement' ? 'selected' : ''; ?>>Announcement</option>
            </select>
        </div>
