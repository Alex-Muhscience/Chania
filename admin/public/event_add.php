...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif (strtotime($formData['event_date']) < strtotime('today')) {
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $db->prepare("INSERT INTO events (title, description, event_date, start_time, end_time, location, max_participants, registration_deadline, is_active, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            $stmt->execute([
                $formData['title'],
