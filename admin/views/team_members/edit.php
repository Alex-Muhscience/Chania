    ...
    <div class="col-md-10">
        <div class="card shadow">
            <div class="card-header">
                <h5 class="mb-0"><i class="fas fa-user-edit"></i> Edit Team Member</h5>
            </div>
            <div class="card-body">
                <?php if (!empty($errors)): ?>
                ...
                <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="position" name="position"
                                       value="<?= htmlspecialchars($_POST['position'] ?? $member['position'] ?? '') ?>" required
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="bio" name="bio" rows="4"
                                  placeholder="Enter a brief biography about this team member..."><?= htmlspecialchars($_POST['bio'] ?? $member['bio'] ?? '') ?></textarea>
                            ...
                            <div class="form-group">
                                ...
                                <?php if (!empty($member['image_path'])): ?>
                                    <div class="current-image mb-2">
                                        <label class="text-muted small">Current Photo:</label><br>
                                        <img src="<?= htmlspecialchars($member['image_path']) ?>" 
                                             alt="Current profile photo" class="img-thumbnail rounded-circle" 
                                             style="max-width: 120px; max-height: 120px; object-fit: cover;">
                                        <div class="mt-2">
                                            <div class="custom-control custom-checkbox">
                                                <input type="checkbox" class="custom-control-input" id="removeImage" name="removeImage" value="1">
                                                <label class="custom-control-label text-danger" for="removeImage">
                                                    <small>Remove current photo</small>
                                                </label>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <small class="form-text text-muted">
                                    <?= !empty($member['image_path']) ? 'Upload new photo to replace current one' : 'Upload profile photo' ?> 
                                ...
                                <div class="mt-2">
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" class="custom-control-input" id="isActive" name="isActive" value="1"
                                               <?= ($_POST['isActive'] ?? $member['status'] ?? 'active') === 'active' ? 'checked' : '' ?>>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="socialLinks" name="socialLinks" rows="3"
                                  placeholder='{"linkedin": "https://linkedin.com/in/member", "twitter": "https://twitter.com/member"}'><?= htmlspecialchars($_POST['socialLinks'] ?? $member['social_links'] ?? '') ?></textarea>
                        <small class="form-text text-muted">
                    ...
                    <div class="form-group">
                        <label class="text-muted small">Member Information:</label>
                        <div class="mt-1">
                            <small class="text-muted">
                                Created: <?= date('M j, Y g:i A', strtotime($member['created_at'] ?? 'now')) ?><br>
                                <?php if (!empty($member['updated_at'])): ?>
                                    Updated: <?= date('M j, Y g:i A', strtotime($member['updated_at'])) ?>
                                <?php endif; ?>
                            </small>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    imageInput.addEventListener('change', function() {
        ...
        if (file) {
            ...
            // If user selects new image, uncheck remove current image
    ...
    // If user wants to remove current image, clear file input
    if (removeImageCheckbox) {
