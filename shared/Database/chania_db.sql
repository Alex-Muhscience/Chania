...
CREATE TABLE applications (
    ...
    FULLTEXT idx_applications_search (first_name, last_name, email),
...
-- Team members table with enhanced features
...
-- Partners table with enhanced features
...
-- Admin activity logs table with enhanced tracking
...
('admin', 'admin@skillsforafrica.org', '$2y$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'System Administrator', 'admin', 'active', TRUE, NOW());
...
CREATE TRIGGER programs_update_log AFTER UPDATE ON programs
FOR EACH ROW BEGIN
...
CREATE PROCEDURE GetApplicationStats(IN program_id INT)
BEGIN
    SELECT
...
CREATE PROCEDURE GetUserActivity(IN user_id INT, IN days_back INT)
BEGIN
    SELECT
...
-- GRANT SELECT, INSERT, UPDATE, DELETE ON chania_db.* TO 'app_user'@'localhost';
-- GRANT EXECUTE ON chania_db.* TO 'app_user'@'localhost';
-- FLUSH PRIVILEGES;